...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        caches = []
        dropout_caches = []
        for i in range(self.num_layers - 1):  #减一是因为最后一层要特殊处理。
            W = self.params['W' + str(i + 1)]
            b = self.params['b' + str(i + 1)]
            if self.normalization == None:
                out, cache = affine_relu_forward(x, W, b)
                #ps：此处affine_relu_forward函数返回的是结果值与
                #  affine_forward 和 relu_forward 的cache值
        ...
        #最后一层特殊处理：（无batch-normalization & dropout）
