...
def batchnorm_backward_alt(dout, cache):
    ...
    N = 1.0 * x.shape[0]
    dfdu = dout * gamma
    dfdv = np.sum(dfdu * (x - mean) * -0.5 * var ** -1.5, axis = 0) # ps:由于针对不同数据集（N*D）的x（N）的求和，因此要axis = 0；最后shape为（D,)
    #print(dfdu.shape)
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    #print(mean.shape) #4*1
    #注意保持原矩阵大小不变！！！
    var = np.var(x, axis = 1, keepdims = True) + eps
    std = np.sqrt(var)
    x_norm = ((x - mean) / std)#.reshape(x.shape)
    out = x_norm * gamma + beta
...
def layernorm_backward(dout, cache):
    ...
    dfdu = dout * gamma
    #print(x.shape)
    dfdv = np.sum(dfdu * (x - mean) * -0.5 * var ** -1.5, axis = 1) # # ps:由于针对不同数据集（N*D）的x（N）的求和，因此要axis = 0；最后shape为（D,)
    #print(std.shape)
    #print(dfdu.shape)
    dfdw = np.sum(dfdu * -1 / std, axis = 1) + dfdv * np.sum(-2/N * (x - mean), axis = 1)
    #print(dfdv.shape)
    #print(dfdv.reshape(-1,1).shape)
    dx = dfdu / std + dfdv.reshape(-1,1) * 2/N * (x - mean) + dfdw.reshape(-1,1) / N

    #由于计算过程是对不同x的求和，所以此时求 gamma和beta仍然是axis = 0；
    dgamma = np.sum(dout * x_norm, axis = 0)
    dbeta = np.sum(dout, axis = 0)
...
def dropout_forward(x, dropout_param):
    ...
    if mode == "train":
        ...
        # 而(np.random.rand(*x.shape)数组中仅为0或1.
